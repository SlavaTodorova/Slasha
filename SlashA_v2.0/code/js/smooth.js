...
/*
 * Extracts information from the corpus, calculates frequencies of ngrams and smooths them
 */

// array of Day objects, these are the days for which there is data
var DATA_BY_DATE = new Array();
// the names of the authors, sorted alphabetically
var AUTHORS = null;
// the tsv two dimentional array
var TSV_ARRAY = [];
// collection of documents that lack some annotation
var BAD_DOCS = null;
// the queries from the current session
var QUERIES = null;
// the period of time covered by the corpus in days (it is >= DATA_BY_DATE.length)
var NUM_DAYS = 0;

/*
 * Loads the selected corpus
 * @param {FileList} corpus corpus to load
 */
function loadCorpus(corpus) {
    ...
    var removed = [];
    var letter_index = 0; // letter index in the DATA_BY_DATE.letters array
    // loop through all the files in the corpus and fill the information into the DATA_BY_DATE array
    for (var i = 0; i < num_files; i++) {
        ...
        if (date !== null && author_name !== null) { // leaving the letters without a date out
            ...
            while (index < DATA_BY_DATE.length && date_is_new) {
                if (!(DATA_BY_DATE[index].date > date || DATA_BY_DATE[index].date < date)) { // if the date is not new
                    date_is_new = false;
                    ...
                    while (aut < DATA_BY_DATE[index].authors.length && author_is_new) {
                        if (DATA_BY_DATE[index].authors[aut].name === author_name) {
                            ...
                            DATA_BY_DATE[index].authors[aut].letter_indices.push(letter_index);
                        }
                        aut++;
                    }
                    if (author_is_new) {
                        var author_of_letter = new Author(author_name, [letter_index], num_tokens);
            ...
            // if no date can be read from the file
    ...
    DATA_BY_DATE.sort(compareByDate); // sort the data by Date Array
    addIndices(); // for highlighting
    getGaps(); // add dates where there are gaps
    addMissingAuthors();  // add missing authors // the AUTHORS array is also filled in with this
    ...
    fillGeneral(); // fill in the tsv array
...
function loadXML(aFile) {
    ...
    xhttp.open("GET", objectURL, false);
    xhttp.send();
...
/*
 * Calculates how many days there are between each two days with data
 * Fills the information in the DATA_BY_DATE array
 */
function getGaps() {
...
/*
 * Makes sure there is object for each author in every day in the DATA_BY_DATE array
 */
 ...
 * @param {Author} a one Author object
 * @param {Author} b another Author object
 * @returns {Number}
 */
...
/*
 * Fills the general information about the data in the corpus in the TSV_ARRAY
 */
function fillGeneral() {
    ...
    TSV_ARRAY[0].push("date_All"); // all the dates
    TSV_ARRAY[0].push("all_PerCent"); // what % tokens from the max tokens per day in the corpus 
    TSV_ARRAY[0].push("all_Num"); // number of tokens for each day for the whole corpus
    var authors = AUTHORS;
    for (var a = 0; a < authors.length; a++) {
        TSV_ARRAY[0].push(authors[a] + "_PerCent"); // % from the max tokens per day foe each subset
        TSV_ARRAY[0].push(authors[a] + "_Num"); // number of tokens for each subset of the corpus
    }
    ...
    for (var i = 0; i < DATA_BY_DATE.length; i++) {
        ...
        for (var power = 3; power > 0; power--) { // obtaining year in format yyyy
            if (year < Math.pow(10, power)) {
                year = "0" + year;
            }
        }
...
function countOccurrences(query) {
    ...
    if (query.subset === -1) {
        ...
        for (var i = 0; i < DATA_BY_DATE.length; i++) {
            ...
            if (DATA_BY_DATE[i].authors[a].letter_indices !== null) {
                for (var x = 0; x < DATA_BY_DATE[i].authors[a].letter_indices.length; x++) {
                    var indices = []; // indexes for highlighting
                    DATA_BY_DATE[i].letters[x].occurrences = 0;
                    // TO DO make lemmas and tags optional
                    var tokens = DATA_BY_DATE[i].letters[DATA_BY_DATE[i].authors[a].letter_indices[x]].tokens;
                    var lemmas = DATA_BY_DATE[i].letters[DATA_BY_DATE[i].authors[a].letter_indices[x]].lemmas;
                    var pos = DATA_BY_DATE[i].letters[DATA_BY_DATE[i].authors[a].letter_indices[x]].tags;
                    for (var c = 0; c < tokens.length; c++) {
                        if (query.items[0].lemma) { // if the query is for lemmas
                            var corpus_word = lemmas[c];
                        } else { // if the query is for a particular form
                        ...
                        if (findMatch(search_word, corpus_word)) {
                            if (findMatch(query.items[0].pos, pos[c])) {
                                ...
                                if (n_gram_found) {
                                    ...
                                    DATA_BY_DATE[i].letters[DATA_BY_DATE[i].authors[a].letter_indices[x]].occurrences++;
                                }
                            }
                        }
                    }
                    if ((DATA_BY_DATE[i].letters[DATA_BY_DATE[i].authors[a].letter_indices[x]].tokens.length - (query.items.length - 1)) > 0) {
                        DATA_BY_DATE[i].letters[DATA_BY_DATE[i].authors[a].letter_indices[x]].frequency = DATA_BY_DATE[i].letters[DATA_BY_DATE[i].authors[a].letter_indices[x]].occurrences / (DATA_BY_DATE[i].letters[DATA_BY_DATE[i].authors[a].letter_indices[x]].tokens.length - (query.items.length - 1));
                    } else if (DATA_BY_DATE[i].letters[DATA_BY_DATE[i].authors[a].letter_indices[x]].tokens.length < 1) {
                        DATA_BY_DATE[i].letters[DATA_BY_DATE[i].authors[a].letter_indices[x]].frequency = null;
                    } else {
                        DATA_BY_DATE[i].letters[DATA_BY_DATE[i].authors[a].letter_indices[x]].frequency = 0;
                    }
                    // for highlighting
                    DATA_BY_DATE[i].letters[DATA_BY_DATE[i].authors[a].letter_indices[x]].highlights.push(new Highlight(query.input, indices));
                    // author frequencies
                    DATA_BY_DATE[i].authors[a].occurrences += DATA_BY_DATE[i].letters[DATA_BY_DATE[i].authors[a].letter_indices[x]].occurrences;
                }
            }
            occs.push(DATA_BY_DATE[i].authors[a].occurrences);
            if ((DATA_BY_DATE[i].authors[a].tokens - (query.items.length - 1)) > 0) {
                DATA_BY_DATE[i].authors[a].frequency = DATA_BY_DATE[i].authors[a].occurrences / (DATA_BY_DATE[i].authors[a].tokens - DATA_BY_DATE[i].authors[a].letter_indices.length * (query.items.length - 1));
...
/*
 * Searching the corpus for n-grams using regular expressions
    ...
    for (var d = 0; d < DATA_BY_DATE.length; d++) {
        ...
        if (DATA_BY_DATE[d].authors !== null) {
            for (var a = 0; a < DATA_BY_DATE[d].authors.length; a++) {
                if (DATA_BY_DATE[d].authors[a].letter_indices !== null) {
                    for (var l = 0; l < DATA_BY_DATE[d].authors[a].letter_indices.length; l++) {
                        if (DATA_BY_DATE[d].letters[DATA_BY_DATE[d].authors[a].letter_indices[l]].text !== null) {
                            DATA_BY_DATE[d].letters[DATA_BY_DATE[d].authors[a].letter_indices[l]].indices = makeIndices(DATA_BY_DATE[d].letters[DATA_BY_DATE[d].authors[a].letter_indices[l]].text, DATA_BY_DATE[d].letters[DATA_BY_DATE[d].authors[a].letter_indices[l]].tokens);
                        } else {
                            DATA_BY_DATE[d].letters[DATA_BY_DATE[d].authors[a].letter_indices[l]].indices = [];
                        }
                    }
                }
            }
 ...
 * @param {type} text the whole text of the letter
 * @param {type} tokens single tokens to get the indexes of
 * @returns {Array} array of arrays of the form [start index, end index] for each token
 */
...
/*
 * Counts how many letters are written by this author
...
function countLetters(author_ind) {
    ...
    while (d < DATA_BY_DATE.length && letters_by_author < 2) {
        if (DATA_BY_DATE[d].authors[author_ind].letter_indices !== null) {
            letters_by_author++;
        }
        d++;
    }
    var author_name = AUTHORS[author_ind];
...
/*
 * Fills the tsv Array with token frequences
 * @param {Atring} word search word
 ...
 * @param {String} word search n-gram
 * @param {Number} parameter smoothing parameter
 * @param {Number} n this is the "n" from "n-gram"
 */
...
/*
 * Gets (from the TSV) the tokens for the whole corpus or a subset for each day in the DATA_BY_DATE array
 * @param {Number} subset the index of the subset, -1 for the whole corpus
 * @returns {Array} the numbers of tokens
 */
...
/*
 * Gets (from the TSV) the occurences of an n-gram
 * @param {Query} query the current query
 * @returns {Array} array of numbers of occurences
 */
...
/*
 *  * Gets (from the TSV) the relative frequencies of an n-gram
 * @param {Query} query
...
/*
 * Testing periods of length (parameter * 2 + 1) 
 * for containing significantly higher/lower frequencies than the rest of the corpus
...
function significantFluctuations(parameter) {
    for (var q = 0; q < QUERIES.length; q++) {
        ...
        if (TSV_ARRAY[0].indexOf(tsv_entry_high) < 0 ||
            ...
            // we don't want too big periods to be tested, it looks confusing
            if (parameter * 2 + 1 < ((DATA_BY_DATE[last_possible].date - DATA_BY_DATE[first_possible].date) / 86400000) / 2) {
                ...
                while (day_ind <= last_possible) {
                    ...
                    if (last_insign !== null && test_int.isSame(last_insign)) {
                        ...
                        if (test_int.occ >= 5 && sum_all_occ - test_int.occ >= 5 && sum_all_occ >= 20) {
                            ...
                            if (test_int.p < 0.01) {
                                if (test_int.occ / test_int.tok > (sum_all_occ - test_int.occ) / (sum_all_tokens - test_int.tok)) {
                                    var result = fluctuationsUpwards(sum_all_tokens, sum_all_occ, test_int);
                                    if (result < 0.01) {
                                        all_high.push(day_ind);
                                        last = "high";
                                    }
                                } else if (test_int.occ / test_int.tok < (sum_all_occ - test_int.occ) / (sum_all_tokens - test_int.tok)) {// one tailed test - downwards
                                    var result = fluctuationsDownwards(sum_all_tokens, sum_all_occ, test_int);
                                    if (result < 0.01) {
                                        all_low.push(day_ind);
                                        last = "low";
                                    }
...
/*
 * Get the distance between two dates in days
 * @param {type} index1 index of the relevant Day object in the DATDATA_BY_DATE array
 * @param {type} index2 index of the other relevant Day object in the DATDATA_BY_DATE array
 * @returns {Number} the distance between two dates in days
 */
 ...
 * @param {Number} ind index of the Day object for which a value has to be interpolated
 * @returns {Number} linearly interpolated frequency
 */
function interpolate(freqs, ind) {
    // go back to the last day with data
    var data_before = ind;
...
/*
 * The iterative part of a Fisher test upwards
 * @param {Number} sum_all_tokens the sum of all tokens in the corpus (or subset)
 * @param {Number} sum_all_occ the sum of all occurencies of the n-gram in the corpus (or subset)
 * @param {Interval} interval the interval of interest
 * @returns {Number} the p value
 */
function fluctuationsUpwards(sum_all_tokens, sum_all_occ, interval) {
...
/*
 * The iterative part of a Fisher test downwards
 * @param {Number} sum_all_tokens the sum of all tokens in the corpus (or subset)
 * @param {Number} sum_all_occ the sum of all occurencies of the n-gram in the corpus (or subset)
 * @param {Interval} interval the interval of interest
 * @returns {Number} the p value
 */
function fluctuationsDownwards(sum_all_tokens, sum_all_occ, interval) { //TODO toks and occs are not needed here!
    var tmp_interval = interval;
    var change = 1;
...
/*
 * Main function for the Fisher test (fluctuations) 
 * @param {Number} all_tok
 * @param {Number} all_occ
 * @param {Interval} an_interval
...
/*
 * Calculates result of the Fisher test
 * @param {Number} a 
 * @param {Number} b 
 * @param {Number} c 
 * @param {Number} d 
 * @param {Number} n all tokens
 * @returns {Number} the p value
 */
...
/*
 * Main function for the Fisher test (differences) 
 * @param {Interval} interval_1
 * @param {Interval} interval_2
 * @returns {Number} p value
 */
...
/*
 * The itarative par for the Fissher test in the case of differences
 * @param {Interval} interval_1
 * @param {Interval} interval_2
 * @returns {Number} p value
 */
 ...
 * @param {String} word search n-gram
 * @param {Number} parameter smoothing parameter
 * @param {Number} aut index of author
 * @param {Number} n this is the n from ngram
 */
...
/*
 * Teting all periods with length wow*2 + 1
 * if the two subsets specified are significantly different in this period
 * @param {Number} wow the smoothing parameter
 */
            ...
            while (day_ind <= last_possible) {
                ...
                if (test_int_1.tok !== 0 && test_int_2.tok !== 0) {
                    ...
                    if (last_insign_1 !== null && test_int_1.isSame(last_insign_1) && test_int_2.isSame(last_insign_2)) {
                        ...
                        if (test_int_1.occ >= 5 && test_int_2.occ >= 5 && (test_int_1.occ + test_int_2.occ) >= 20) {
...
/*
 * Check if the queries are suitable for the sign diffs test
 * @returns {Boolean}
 */
